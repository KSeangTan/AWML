"""Plot histograms of LiDAR point counts for matched predictions from evaluator.pkl."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "predictions at a given matching threshold."
        )
    )
    parser.add_argument(
        "evaluator_pkl",
...
def plot_histograms(
    pred_counts: DefaultDict[str, DefaultDict[str, List[int]]],
    output_dir: Path,
    matching_mode: MatchingMode,
    threshold: float,
    bins: int,
    ...
    for evaluator_name, class_to_counts in sorted(pred_counts.items()):
        ...
        fig.suptitle(
            f"{evaluator_name}\n"
            f"Matched predictions ({matching_mode.value}, threshold={threshold:g} m)",
            fontsize=14,
            y=0.995,
        )
        ...
        for row_idx, class_name in enumerate(class_names):
            ...
            if counts.size == 0:
                ax.set_title(f"{class_name} (no matched predictions)")
                ax.axis("off")
                continue
        ...
        output_path = output_dir / f"matched_pred_pointcloud_hist_{safe_name}.png"
...
def main() -> None:
    ...
    saved_paths, all_stats = plot_histograms(
    ...
    print("\nPrediction pointcloud_num statistics:")
    for evaluator_name in sorted(all_stats.keys()):
        print(f"  [{evaluator_name}]")
        for class_name in _sorted_classes(all_stats[evaluator_name].keys()):
            s = all_stats[evaluator_name][class_name]
            print(
                f"    {class_name}: min={s['min']:.0f} p25={s['p25']:.0f} p50={s['p50']:.0f} "
